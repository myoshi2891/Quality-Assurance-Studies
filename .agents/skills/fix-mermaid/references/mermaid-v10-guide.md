...
1. 先頭の HTML インデントをすべて除去する（**例外: mindmap** — 先頭の `%%{init}%%` などのディレクティブは `%%{` で始まる行から `}%%` を含む行までの複数行全体を（1 行の `%%` コメントもあわせて）読み飛ばし、その後の最初の非空行が `mindmap` で始まるブロックは、内部のインデントが階層構造を表す Mermaid 構文であるため除去しない）
