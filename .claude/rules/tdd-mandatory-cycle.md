...
- **HTML移行・新機能の場合**: 移行着手前に元HTMLの全構成要素（全見出し、全TOCリンク、全Mermaid図解、全表、全コードブロック、全コールアウト、全参考文献）の棚卸しインベントリを作成する。そして、これらの構成要素がすべて正しくレンダリングされていることを1対1でアサーションする網羅的テストファイルを `tests/` 下に作成する（抜け漏れゼロの担保）。
