...
```bash
# 全画面ヒーローを意図しているページ（ダーク系ランディング風）はそのまま実行する
bun scripts/check-globals-interference.mjs /<page-slug>

# カード型ヒーロー（原本 HTML が height を指定していないヒーロー）のページのみ --card-hero を付ける。
# 付けないと .hero の 100vh / flex 中央寄せ判定が info 扱いのままで見逃される（警告に昇格しない）。
...
#### 見た目の微調整で同じ指摘が2回続いたら、値のチューニングをやめる【最重要・頻発パターン】

このプロジェクトはブラウザ自動操作（Playwright等）の使用が禁止されているページが多く、AIエージェントは
自分が書いたCSSの実際のピクセルレンダリングを目視確認できない。そのため「ユーザーのスクリーンショット報告 →
値を推測で調整 → 反映されない/違う」というループに陥りやすい（2026-09 `istqb-ctal-ta-chapter1-test-process`
のサイドバー境界線で、`--border-strong` 2px → 1px → 元の `--border` 1px 復元、と3回連続で色/太さを
推測変更し、いずれもユーザーから「まだ黒くて太い」と差し戻された実例がある）。

**ルール**: 同じ視覚要素について、値を変えた修正が**2回**ユーザーから「まだ直っていない」と差し戻されたら、
3回目の値当てずっぽうを試みてはならない。代わりに次のどちらかを直ちに行う:

1. **要素を完全に削除・無効化する**（色や太さの再調整ではなく）。特にユーザーが「不要」「要らない」
   と言っている場合、忠実移行（元HTMLとの一致）より**ユーザーの明示的な指示を優先する**。元HTMLに
   存在する要素でも、ユーザーが視覚的に不要と判断すれば削除してよい。
2. どうしても値の調整が必要な場合は、ユーザーに **DevTools の Computed スタイルのスクリーンショット**
   （実際に適用されている色・太さの実測値）を1回求めてから、その実測値に基づいて修正する。
   推測に基づく3回目の変更を提案しない。

**やってはいけないこと**: 「今度こそ直りました」を検証済みの根拠なく3回以上繰り返すこと。
`.next` の再ビルド確認とコンパイル済みCSSの `grep` は「自分が書いた値が反映されているか」の確認にしか
ならず、「ブラウザで見た目がユーザーの期待通りか」の確認にはならないことを常に意識する。両者を混同しない。
