...
| Mermaid エッジラベルの黒潰れ/色崩れ | `Mermaid.tsx` のグローバルダークテーマ設定や SVG 内部構造（`foreignObject`, `rect`）と競合し、分岐テキスト（はい/いいえ）が黒四角に潰れる | ページ固有 CSS で**背景と文字を別セレクタに分けて**指定する。矩形（`.mermaid-wrapper .edgeLabels rect`）には `fill: var(--card) !important;`（または `fill: transparent !important;`）と `stroke: none !important;` を当て、文字（`.mermaid-wrapper .edgeLabel`, `.mermaid-wrapper .edgeLabel span`, `.mermaid-wrapper .edgeLabel text`）には `background-color: var(--card) !important; color: var(--ink) !important; fill: var(--ink) !important; font-weight: 700 !important; overflow: visible !important;` を当てる。**`rect` に `fill: var(--ink)` を当ててはならない**（矩形がインク色で塗り潰され、文字が黒潰れする原因そのものになる） |
...
- **Always style Mermaid edge labels cleanly without dark box artifacts** — Mermaid の分岐ラベル（はい/いいえ等）は、共通ダークテーマや SVG 内部構造（foreignObject, rect）の競合により黒潰れや意図しない背景色になりやすい。ページ固有 CSS で矩形（`.mermaid-wrapper .edgeLabels rect`）には背景色（`fill: var(--card)` またはカード同化・透明）と枠線除去（`stroke: none`）を、文字（`.mermaid-wrapper .edgeLabel`, `.mermaid-wrapper .edgeLabel span`, `.mermaid-wrapper .edgeLabel text`）には高コントラストなテキスト色（`color` / `fill: var(--ink)`）を、**セレクタを分けて**指定すること。矩形の `fill` にインク色を当ててはならない。
