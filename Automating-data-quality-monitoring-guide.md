...
flowchart LR
    ...
    predict -->|当てられない| ok["明確な変化は検出されず<br/>（サンプル・特徴量・モデル性能の範囲内）"]
```

もしモデルが「今日のデータかどうか」を高い確度で当てられるなら、それは今日のデータが過去のパターンと明確に異なっているという意味であり、何らかの異常が疑われます。逆に見分けがつかない場合は、扱ったサンプル・特徴量・モデルの性能の範囲内では明確な変化が検出されなかったことを意味するにとどまり、異常が存在しないことの証明にはなりません[^8]。
