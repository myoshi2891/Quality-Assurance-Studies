...
『Beautiful Testing』刊行(2009年)後の10年余りで、テスト戦略を可視化する「地図」がいくつも生まれました。初学者はまずこの地図を知っておくと、大量にあるテストの種類を迷わず整理できます。

下図は、後述する3つのモデルを統合した唯一の正解モデルでも、必ずこの順に実施しなければならないという規範でもありません。**CIパイプラインで各種テストを実行する順序の一例**として示すものです（速く安く失敗を見つけられるものから先に流す、という考え方）。それぞれのモデルの違いは、直後の比較表で整理します。

```mermaid
flowchart TB
    Static["静的解析<br/>Lint / 型チェック"] --> Unit["ユニットテスト<br/>Small"]
    Unit --> Integration["統合テスト<br/>Medium"]
    Integration --> E2E["E2Eテスト<br/>Large"]
    E2E --> Manual["探索的テスト<br/>手動"]
```

代表的な3つのモデルを比較します。これらは互いに排他的な選択肢であり、上図のような実行順序とは別の観点（何をどれだけ書くべきかの配分）を示すものです。
...
- **いきなり大きなテストを書こうとしてしまう**
  → 「1つのテストで1つの振る舞いだけを検証する」というAAAパターン（Arrange＝準備、Act＝実行、Assert＝検証）を徹底し、小さく刻む。
- **実装を先に書いてからテストを後付けしてしまう**
  → まず失敗するテスト（Red）を書き、それが正しい理由で失敗することを確認してから実装に進む。
- **リファクタリングを省略してしまう**
  → テストがGreenの状態は「安全網が張られた状態」なので、このタイミングでこそ設計を整理する。

原著が強調するのは、TDDで書かれたテストは単なる検証コードではなく、**「読める仕様書（Readable Examples）」であり「恒久的な要求仕様の記録（Permanent Requirement Artifacts）」**になるという点です。テストコードを書くときは、「後で読む人（未来の自分やチームメイト）が仕様を理解できるか」を常に意識しましょう。
...
|---|---|---|
| **開発を支援する（Supporting the team）** | Q2: ストーリーテスト（例示ベースの受け入れ基準テスト。例: Cucumber, FitNesse）。**開発前に「何を作るか」を具体例で合意し、実装を導く**ために書き、自動化して繰り返し実行する | Q1: ユニットテスト・コンポーネントテスト（TDDの土台） |
| **製品を批評する（Critiquing the product）** | Q3: 探索的テスト・ユーザビリティテスト・ユーザー受け入れテスト（UAT）。**動くものに対してユーザー視点で「本当に価値があるか」を批評する**ために行い、多くは人手による探索・評価が中心 | Q4: 性能・セキュリティなど非機能要件のテスト |

> 同じ「受け入れテスト」という語がQ2とQ3の両方に現れますが、意図と進め方は異なります。Q2は開発を前に進めるための**例示ベースのストーリーテスト**、Q3は完成した製品を利用者視点で吟味する**批評としてのUAT**です。

初学者は、まず「自分が今書こうとしているテストはこの4象限のどこに位置するか」を意識するだけで、自動化すべきか、人手で探索すべきかの判断がしやすくなります。探索的テストは「行き当たりばったりのテスト」ではなく、**仮説を立てて検証しながら学習する、規律あるプロセス**であることを覚えておきましょう。
...
本章はAndreas ZellerとDavid Schulerによる研究（Javalancheフレームワーク）が土台になっているとみられ、「意図的にコードへ小さなバグ（ミューテーション）を仕込み、既存のテストスイートがそれを検出できるかどうかでテストの"強さ"そのものを測定する」という発想を紹介します。カバレッジ（実行された行数の割合）だけでは測れない、**「本当にテストがバグを捕まえられるか」**を検証する手法です。
...
- [ ] プロジェクトの性質に合わせて、テストピラミッド／トロフィー／テストサイズのどれを基準にするか決めた
