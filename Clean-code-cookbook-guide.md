...
リファクタリングの定義は「振る舞いを変えないこと」でした。振る舞いが変わっていないことを機械的に保証する手段がテストです。テストが存在しない、または不十分な場合は、まず現状の振る舞いを固定する「特性化テスト」を先に書きます。
...
flowchart TB
    ...
    Later --> Impl
    Impl --> Commit["振る舞いを変えずにテストしてコミットする"]
```
...
| コミット単位 | 変更は小さく、振る舞いを変えずにテストが通る単位でコミットされているか |
