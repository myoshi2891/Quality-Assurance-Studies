...
- **基盤LLM（Foundation LLM）**：幅広いテキストデータで事前学習された汎用モデルです。要約・翻訳・質問応答など多様な言語タスクに対応できる柔軟性を持ちますが、強力である一方、特定タスクに適用するにはさらなる調整が必要になることが一般的です（画像や音声も扱えるモデルは、1.1.4 で扱うマルチモーダルLLMとして区別されます）。
