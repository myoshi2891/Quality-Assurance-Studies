...
CT-GenAI試験は、ISTQB® Certified Tester Foundation Level（CTFL）取得を前提資格とするスペシャリストレベルの認定です。試験は**40問・合格ライン30点（65%）・制限時間60分**（非母語受験者は+25%）で構成されており、全5章のうち、この第1章はシラバス全体（約13.6時間）の中で**100分**が割り当てられた「導入」にあたる章です。
...
| GenAI-1.1.4 | K2（理解） | マルチモーダルLLMとVision-Language Modelの基本原則を要約できる |
| HO-1.1.4 | H1（実践演習） | テキストと画像入力を用いたマルチモーダルLLM向けプロンプトを作成・実行する |
...
これら4つは新しい技術が古い技術を置き換えるものではなく、それぞれ異なる強みと限界を持つ並存的な技術群です。生成AIの最大の利点は、追加の学習フェーズを必要とせず、事前学習済みのモデルをそのままテストタスクに適用できる点にありますが、これには相応のリスクも伴います（リスクの詳細は第3章で扱います）。
...
| 生成AI | 深層学習の応用 | 事前学習済みモデルの活用（追加学習は不要） | テストケース・スクリプト・レポートの自動生成 |

> 💡 **ベストプラクティス**
> - 「なぜ生成AIがテスト業務に向いているか」を説明する際は、**追加の学習フェーズが不要**という利点を軸に説明すると、他の機械学習手法との違いが明確になる。
...
大規模言語モデル（LLM）は、Generative Pre-trained Transformer（GPT）という深層学習モデルをベースに、書籍・記事・Webサイトなど非常に大規模なデータセットで学習されています。パラメータ数を絞り、軽量かつ特定用途に特化させたモデルは**小規模言語モデル（SLM）**と呼ばれます。
...
- **基盤LLM（Foundation LLM）**：テキスト・コード・画像など多様かつ膨大なデータセットで学習された汎用モデルです。自然言語処理・コンピュータビジョン・音声認識など幅広いタスクを支援できる柔軟性を持ちますが、強力である一方、特定タスクに適用するにはさらなる調整が必要になることが一般的です。
...
シラバスの演習では、テキストと画像の両方を入力とした既存のプロンプトをレビューし、実際にマルチモーダルLLMで実行して結果を検証する、という2ステップを踏みます。これにより、テキストと画像を組み合わせたテストタスクにおけるマルチモーダルLLM活用の利点と課題の両方を体感できます。
...
テスターがチャットボットを使うか、統合されたLLM搭載アプリケーションを使うかにかかわらず、生成AIをテストにうまく取り入れるには**強力なプロンプトエンジニアリング**が欠かせません。丁寧に設計されたプロンプトと明確で具体的な指示は、LLMが生成する出力が正確で、関連性があり、テストの目的に沿ったものであることを保証するために不可欠です。
