...
| HO-1.1.4 | H1（実践演習） | 所与のテストタスクに対してテキストと画像入力を用いるプロンプトを作成・記述し、その入力をレビューしたうえで、作成したプロンプトをマルチモーダルLLMで実行する |
...
|---|---|---|---|
| 基盤LLM | 多様な大規模データで事前学習のみ | 汎用性が高く多様なタスクに対応できる。特定タスクには追加調整が必要な場合がある | 汎用的な文章要約・下書き作成 |
...
シラバスの演習では、(1) 与えられたテストタスクに対して、テキストと画像の両方を入力とするプロンプトをまず自分で作成・記述し、(2) そのプロンプトと入力データをレビューしたうえで、マルチモーダルLLMで実行して応答を確認・検証する、という2ステップを踏みます。これにより、テキストと画像を組み合わせたテストタスクにおけるマルチモーダルLLM活用の利点と課題の両方を体感できます。
