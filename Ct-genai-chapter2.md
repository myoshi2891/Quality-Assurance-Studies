...
>
> **バージョンについての注記**：現行の最新シラバスは **v1.1（2026年4月27日リリース）** です。v1.1は v1.0 に対する **マイナーアップデート**（章立て・学習目標・出題範囲は変更なし）であり、用語の一部修正や説明文の微調整が中心です。本ガイドは章の内容を v1.0 の全文（英語原文）に基づいて詳細に解説し、v1.1 で変更された箇所は本文中および「6. v1.0→v1.1 変更点」に明記しています。正確な最終文言は必ず上記の公式シラバスでご確認ください。
...
前節で学んだ6要素構造と3つのコア技法を、実際のテストプロセスの各局面に適用していきます。プロンプトチェイニング・Few-shotプロンプティング・メタプロンプティングの中からタスクに応じて単独または複数の技法を選び、単一の技法では不足する場合に限って組み合わせることで、チームはテスト目的に合わせてAIプロンプトを調整し、より正確で関連性が高く効果的な出力を得ることができます。**高品質な入力（Input Data）が、意味のあるAI結果を得るために不可欠**である点は、全ての局面に共通する重要な前提です。
...
flowchart TD
    ...
    Q3 -->|"いいえ"| COMB["単一技法では不足する場合に限り<br/>複数技法の組み合わせを検討"]
...
> ⚠️ **v1.1での変更に関する注記**：上表の「実行成功率（Execution Success Rate）」は **v1.1 の定義**（生成されたテスト成果物のうち、テスト環境でそのまま実行できるものの割合）に基づいています。v1.0 では「生成されたテストケースまたはテストスクリプトのうち、正常に実行できるものの割合」と定義されており、v1.1 のリリースノートでこの説明文の改訂が明記されています。正確な最新の文言は公式シラバスをご確認ください。
...
| 2.3.1 表内「Execution Success Rate（実行成功率）」の説明文 | 生成されたテストケースまたはテストスクリプトのうち、正常に実行できるものの割合 | 生成されたテスト成果物のうち、テスト環境でそのまま実行できるものの割合に改訂（本ガイド3.1節の表は v1.1 の定義に準拠） |
