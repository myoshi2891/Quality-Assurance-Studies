...
合計 13.6 時間（815 分）は、認定トレーニングコース（accredited course）に求められる最低講義時間です。個人の試験準備に推奨される学習時間を示すものではありません。
...
| テストデータ合成 | 現実的な合成テストデータを生成（プライバシー保護は生成方法に依存し、再識別リスクなどを別途評価する必要がある） |
...
| 攻撃ベクトル | 説明 | 具体例 |
|---|---|---|
| コンテキスト操作 | 機密の学習データを引き出すよう設計されたリクエストの送信 | 学習時に記憶された内容をクエリで引き出す学習データ抽出を狙う（なお、コンテキスト上限の超過自体は漏えいの原因ではなく、入力の切り詰めやリクエストの拒否につながる場合があるという別の現象である） |
| リクエスト操作 | AIの出力を混乱させるデータの投入 | 画像でAIを別の文脈に誘導し、受け入れ基準等でハルシネーションを誘発 |
| データポイズニング | 学習データの改ざん | AI生成テストレポートの結果を評価する際に偽の評価を与え、それが学習データに取り込まれることでモデルの推奨内容を操作する |
...
- **担当者向け研修**：責任あるGenAI利用のための明確な教育プログラムとポリシーを策定
...
| ISO/IEC 23053:2022（標準） | 機械学習を用いるAIシステムを記述するための概念的枠組み（構成要素と機能）と共通用語を規定 | GenAIテストにおけるデータ品質・透明性・安全性の枠組みを提供（CT-GenAI Syllabus v1.1 の表記は「安全性（safety）」、Sample Exam A 解答 Q28 の表記は「耐障害性（fault tolerance）」で、公式資料間で用語が異なる。試験対策では各資料に記載された表現に沿って扱う） |
...
ソフトウェアテストにおいてRAGは、LLM搭載テストインフラが企業内のデータベース・ドキュメント・リポジトリへ事前に構築したベクトルデータベースのインデックスを介してアクセスすることを可能にします。テスト分析やテスト設計にソースの更新が反映されるのは、データ取り込みとインデックスが適切な頻度で同期されている場合に限られ、同期が遅れると古い仕様・要件に基づく可能性があります。
...
| コンプライアンス・規制上の問題 | コンプライアンス検証を経ていないAIツールの利用が業界標準・規制への違反を招く | GDPRの適用対象となる個人データを含むテストデータを、処理の適法根拠や必要な保護措置（処理者契約・域外移転の保護措置など）がないままAIツールへ送信し、GDPR違反となる |
