...
flowchart LR
    subgraph CH1["第1章の3本柱"]
        ...
        P3["1.3 成果物に関するタスク<br/>7つの成果物とツール"]
    end
    P1 --> P2 --> P3
...
|---|---|---|---|
| **順次(ウォーターフォール型)** | フェーズ間の重複が少なく、前フェーズ完了後に次フェーズが開始 | 時間の経過とともにタスクの種類が変わる(計画支援→分析→設計/実装→実行) | 比較的低い(1回のリリースサイクルが長い) |
...
**なぜ重要か:** テスト環境の実装品質は、テスト容易性・欠陥検出力・総テストコスト・**テスト結果の信頼性**に直接影響します。理想的なテスト環境は「テスト環境で合格/不合格になった結果が本番でも同じ結果になる」ことを保証できるものです。
