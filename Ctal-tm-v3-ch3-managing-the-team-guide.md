...
| 🟡 | 一般的な実務知識・ISTQB 用語集・二次情報で補足した内容。第3章の本文（シラバス p.65〜73）で表現や範囲を必ず確認してください |

> ⚠️ 重要な注意
> 本ガイドの作成時、公式シラバス PDF の取得が第1章の途中（p.45 付近）で打ち切られたため、第3章の本文（p.65〜73）そのものは原文で確認できていません。
> 第3章の節構成・学習時間・章の狙い・第1章からの参照は 🟢 ですが、各節の詳細説明は 🟡 を含みます。
> 試験対策として使う前に、日本語版シラバス（JSTQB 版 Version 3.0.J02）の p.65〜73 と突き合わせてください。URL は「9. 出典」にあります。
...
- [ ] 日本語版シラバスの p.65〜73 と本ガイドの 🟡 部分を突き合わせた
...
| 資料 | URL | 用途 |
|------|-----|------|
| JSTQB：Advanced Level シラバス日本語版 テストマネジメント Version 3.0.J02 | https://jstqb.jp/dl/JSTQB-Syllabus.Advanced_TM_VersionV3.0.J02.pdf | 第3章（p.65〜73）の日本語訳の確認 |
...
- 🟡 の内容は、ISTQB 用語集と一般的な実務知識に基づく補足です。第3章本文（p.65〜73）の表現・粒度と異なる可能性があります
