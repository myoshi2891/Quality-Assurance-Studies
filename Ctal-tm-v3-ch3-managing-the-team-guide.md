...
| 🟡 | 一般的な実務知識・ISTQB 用語集・二次情報で補足した内容。第3章の本文（シラバス p.62〜70）で表現や範囲を必ず確認してください |

> ⚠️ 重要な注意
> 本ガイドの作成時、公式シラバス PDF の取得が第1章の途中（p.45 付近）で打ち切られたため、第3章の本文（p.62〜70）そのものは原文で確認できていません。
> 第3章の節構成・学習時間・章の狙い・第1章からの参照は 🟢 ですが、各節の詳細説明は 🟡 を含みます。
> 試験対策として使う前に、日本語版シラバス（JSTQB 版 Version 3.0.J02）の p.62〜70 と突き合わせてください。URL は「9. 出典」にあります。
...
シラバスは「テストチームメンバー（test team member）」を、組織のコンテキストや他の役割に関係なく、テストマネジメントまたはテストをする役割でテストを実施するすべての人と定義しています。
...
|----------|------|--------------|----------------------|
| 専門的能力 | Professional competence | 何を知っているか（知識） | テスト対象ドメインの知識、技術スタック、テスト対象システムのアーキテクチャの理解、規制・標準の知識 |
| 方法論的能力 | Methodological competence | どうやって進めるか（方法） | テスト技法、テスト設計、見積り、リスク分析、テストツールや自動化の活用、計画立案、課題の構造化 |
...
| 種類 | 要因の例 |
|------|----------|
| 動機付け要因 | 明確な目標と意義の共有、成果への承認、スキル成長の機会、裁量と自律性、信頼できる仲間、適切なツールと環境、公正な評価 |
| 意欲低下要因 | 目標や優先順位の頻繁な変更、テストが軽視される文化、過度な残業と締切圧力、成果に見合わない評価、不明確な役割、頻繁なやり直し、スキルの停滞、批判的で心理的安全性のない雰囲気 |
...
ISTQB 用語集では、品質コスト（cost of quality）を、品質に関する活動や問題に費やした総コストであり、多くの場合、予防コスト、評価（検出）コスト、内部失敗コスト、外部失敗コストに分けられるもの、と定義しています。
...
| 評価（検出）コスト | Appraisal (detection) cost | 欠陥を見つけるための活動 | テストの設計と実行、レビュー、テスト環境・テストツールの費用 |
...
| リリース前コスト（投資） | 予防コスト ＋ 検出コスト ＋ 内部失敗コスト |
| 正味便益 | 便益 − 投資 |
| 費用対効果（ROI） | 正味便益 ÷ 投資 |
...
**問3** 予防コスト 100 万円、想定欠陥 40 件、検出コスト 1 件 2 万円、内部失敗コスト 1 件 3 万円、外部失敗コスト 1 件 30 万円の場合、正味便益はいくらか（テストで見つかる欠陥はすべて、テストがなければ本番で見つかると仮定する）。
...
- [ ] 日本語版シラバスの p.62〜70 と本ガイドの 🟡 部分を突き合わせた
...
| 資料 | URL | 用途 |
|------|-----|------|
| JSTQB：Advanced Level シラバス日本語版 テストマネジメント Version 3.0.J02 | https://jstqb.jp/dl/JSTQB-Syllabus.Advanced_TM_VersionV3.0.J02.pdf | 第3章（p.62〜70）の日本語訳の確認 |
...
- 🟡 の内容は、ISTQB 用語集と一般的な実務知識に基づく補足です。第3章本文（p.62〜70）の表現・粒度と異なる可能性があります
