...
テストマネジメントの主要活動は次の4つで、それぞれにメトリクスが関係します。
...
| コードカバレッジ | テストで実行されたコードの割合 | 表2で○は1列のみ（下の注を参照） |
| テスト活動の実績 対 計画時の見積り（時間単位） | 工数の計画値と実績 | 表2で○は1列のみ（下の注を参照） |
| 欠陥解決数累計 対 欠陥数累計 | 見つかった欠陥のうち解決済みの割合 | 表2で○は1列のみ（下の注を参照） |
| 実際の自動化テストケース 対 計画した自動化テストケース | 自動化の達成度 | 表2で○は1列のみ（下の注を参照） |
| 実際のテストコスト 対 計画したテストコスト | 予算に対する実績 | 表2で○は1列のみ（下の注を参照） |

> ⚠️ **注（確認のお願い）**：表2は「○」の位置（どの列に付くか）で使いどころを示しています。私が参照した本文抽出では、上の3行以外は○が1つだけ付くことは読み取れましたが、**モニタリング側か完了側かの列位置までは確認できませんでした**。試験前に、英語版シラバス p.48〜49 または日本語版 p.51 の表2で列位置を必ず確認してください。考え方の目安は次のとおりです。
> - 「進捗」を示すもの（工数の計画対実績、欠陥の解決状況など）は**モニタリング／コントロール向き**
> - 「目的の達成度」を示すもの（自動化の達成度、終了基準となるカバレッジなど）は**完了向き**
...
期待値は19人日で、およそ「19 ± 5人日（14〜24人日）」の範囲に収まる可能性が高い、と説明できます。悲観的推定値が大きく離れているため、標準偏差が大きく、**不確実性が高いタスク**であることも同時に読み取れます。
...
- 欠陥が**混入したのと同じフェーズ内で取り除ける**（フェーズ内阻止が完全に達成される）とき、品質コストは**最小化**されます。
...
|---|---|---|---|
| 1 | 欠陥の**混入・検出・除去のフェーズ** | **フェーズ内阻止**のアセスメント、**品質コスト分析** | 各フェーズの欠陥検出効果を上げ、欠陥関連コストを最小化する |
...
#### 具体例（架空）：混入フェーズ×検出フェーズのマトリクスで「フェーズ内阻止」を評価する
...
これは「欠陥が最も多く混入するフェーズはどこか」「フェーズ内阻止はどの程度達成できているか」を判断する典型的な使い方です。
...
| 2.3 欠陥マネジメント | 不正→調査→欠陥レポート→ワークフロー。**単純な5状態**（オープン・進行中・却下・解決・クローズ）。委員会とトリアージ。アジャイルは軽量だが5条件で作成。ハイブリッドの課題は**ツール・優先順位・計画の整合性**。必須項目は4つ。フェーズ内阻止と根本原因で改善 | 「欠陥は早く・同じフェーズで・記録して学ぶ」 |
...
| 欠陥のフェーズ内阻止 | 第3章 3.2.1 品質コスト | 早期検出ほど品質コストが下がる |
