...
`GITHUB_TOKEN`は、ワークフロー実行のたびにGitHubが自動生成する一時的なインストールアクセストークンである。2021年により細かい権限モデルが導入され、現在では新規リポジトリ・組織のデフォルトはread-onlyになっているが、既存のリポジトリでは書き込み可能な設定のままになっているケースも多い。
...
これらの事例に共通するのは、「可変な参照(タグ・ブランチ)への依存」が侵害の入口になっている点である。攻撃と検知の間の窓は数日ではなく数時間であることが多いという指摘は、日常的な監視体制の重要性を示している。
