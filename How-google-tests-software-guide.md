...
```mermaid
flowchart TB
    L["Large テスト（目安 約10%）<br/>外部ネットワーク・外部システムの利用を許可<br/>データベース・ファイルシステムへのアクセス可<br/>実行時間：900秒（15分）以上"]
...
| Large | リソース制約なし（マルチプロセス・マルチスレッド可） | 外部ネットワーク・外部システム・データベース・ファイルシステムの利用可 | 900秒（15分）以上 | `long` = 900秒 | 約10% |
