...
```mermaid
flowchart TB
    L["Large テスト（目安 約10%）<br/>複数マシン・本番同等環境で実行<br/>外部ネットワークや実サービス呼び出しを許可<br/>実行時間：900秒（15分）以内"]
...
| サイズ | 実行範囲 | 許可される依存関係 | 実行時間の上限（既定タイムアウト） | 目安の構成比（70/20/10） |
|---|---|---|---|---|
| Small | 単一プロセス内 | 不可（ネットワーク・ディスクI/O・他プロセス禁止） | 60秒以内 | 約70% |
| Medium | 単一マシン内の複数プロセス | localhost通信のみ許可 | 300秒（5分）以内 | 約20% |
| Large | 複数マシン・本番同等環境 | 外部ネットワーク・実サービス呼び出し可 | 900秒（15分）以内 | 約10% |
...
| GTAC（外部カンファレンス） | 2006年から毎年開催されていたが、2017年の開催を最後に休止。アーカイブ動画は現在も公開されている |
