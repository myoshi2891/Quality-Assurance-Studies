...
| FL-6.2.1 | K1（記憶） | テスト自動化とツールサポートの潜在的な利点とリスクを要約できる |
...
**Q1（K1）**: 次のうち、v4.0 シラバスの Chapter 6 のキーワードとして明記されているものはどれか。
A. data-driven testing　B. test automation　C. test comparator　D. scripting language

<details><summary>解答</summary>B. test automation（テスト自動化。v4.0 シラバスの第6章では、キーワードとして 'test automation' のみが明記されています）</details>
