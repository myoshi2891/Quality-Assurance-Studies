...
| 頭文字 | 意味 | 説明 | 具体例 |
|---|---|---|---|
| **C**onfidentiality（機密性） | 秘密にすべき情報を秘密のまま保つこと | 診療記録が第三者に漏れないこと |
| **I**ntegrity（完全性） | データが許可された方法でしか変更されないこと | 選挙の投票結果が改ざんされていないこと |
| **A**vailability（可用性） | 必要なときにデータ・機能が使えること | 消防が火災発生場所の情報に即座にアクセスできること |
| **T**raceability（追跡可能性） | 誰が・いつ・何を変更/参照したかを追跡できること | GDPR（EU一般データ保護規則）が求める監査ログ |
...
一度生成したら状態を変更できないオブジェクトは、複数のスレッドから安全に共有でき、可用性（サービス拒否攻撃への耐性）の観点でも有利です。また、「後から誰かが不正な値に書き換える」という攻撃経路そのものが存在しなくなります。
...
バリデーションには決まった順序があり、これを誤ると（例えば構文チェックを先にしてしまうと）攻撃者に有利な情報を与えたり、無駄な計算資源を消費させられたりします。
...
この順序で軽い検査から重い検査へと進めることで、不正データを早い段階（安価な段階）で弾き、Billion Laughs攻撃のような「パーサーに到達する前に弾くべきもの」を確実に排除できます。
...
- **APIの堅牢化**：外部公開するAPIの引数・戻り値をすべてドメイン・プリミティブにすることで、境界を越えるすべてのデータが自動的に検証される。
...
セキュリティは一度書いて終わりではなく、CI/CDパイプラインの中で継続的に検証すべき対象です。書評でも触れられているように、書籍内のテストコード例はJUnit 5を用いた実践的なものになっており、次のような観点が扱われます。
