...
> 「後工程ほど修正コストが跳ね上がる」という傾向についても、工程別の修正コストを
> 直接測定した一次資料を確認できていないため、本ガイドでは特定の出典に帰属させません。
> 上表は倍率ではなく、工程が進むほど修正に伴う作業の範囲が広がるという定性的な整理です。
> なお NIST が2002年に公表した報告書(参考文献15)は、不十分なテストインフラによる
> 米国経済全体の損失額を試算したものであり、工程別の修正コスト比を示す資料ではありません。
...
> 2026年時点の業界動向を把握するための業界レポートです。15は不十分なテスト
> インフラが米国経済全体にもたらす損失額の試算としてのみ参照しており、工程別の
> 修正コスト比の根拠ではありません(本文「なぜテストが必要なのか」節の注記を参照)。
> 16は継続的インテグレーションの効果に関する主張の根拠として参照しています。
