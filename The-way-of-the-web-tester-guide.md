...
UIが存在しないバックエンドのロジックやAPI連携を検証するのが統合テストです。原著では「Webの仕組み（How the Web Works）」から丁寧に解き明かし、HTTP通信の基本を学んだ上でREST APIのテストへとつなげています。
...
| メソッド | 目的 | べき等性 | テストで確認すべきこと |
|---|---|---|---|
| GET | リソースの取得 | あり | 正しいデータが正しい形式で返るか、存在しない場合は適切なステータスコードか |
| POST | 新規リソースの作成 | なし | 作成が成功したか、重複作成時の挙動、必須項目のバリデーション |
| PUT | 既存リソースの更新（全体置換） | あり | 更新内容が反映されているか、存在しないリソースへのPUTの挙動 |
| DELETE | リソースの削除 | あり | 削除後に該当リソースが取得できなくなっているか |
...
5. 必要に応じて、テスト後に作成・変更したデータをクリーンアップする

> べき等性（Idempotency）とは「同じリクエストを複数回送っても結果が変わらない」性質のことです。GET・PUT・DELETEはべき等、POSTは非べき等という点はテスト設計上の重要な判断材料になります。
...
|---|---|---|
| ロールベースのセレクタを優先する | `getByRole` を第一候補にし、次点で `getByLabel` / `getByText` / `getByTestId` を使う | 「CSSは選択のため」という原則を、アクセシビリティ属性ベースに発展させたもの |
...
| 10 | テスターと開発者が同じ語彙（ピラミッド/トロフィーなどの共通モデル）でテスト戦略を議論できているか |
