...
- 単体テストでは外部サービスへの依存をモックやスタブに置き換え、ネットワークの不安定さを排除する。ただしモック化はその外部連携を検証対象から外すことでもあるため、HTTPのやり取り・認証・シリアライズ・タイムアウト・契約の境界については、実サービスに対するテストや契約テスト（Contract Testing）で別途検証する
...
ただし、「統合テストを厚くする」という結論がそのまま当てはまるとは限らない点に注意してください。何を「統合テスト」と呼ぶか（テスト境界の引き方）は文脈によって大きく異なり、適切な配分はプロジェクトの性質、各層のテストの実行速度と信頼性、そして保守コストによって変わります。トロフィー型が有効なのは、コンポーネント結合レベルのテストが十分に速く安定して書ける環境に限られます。
