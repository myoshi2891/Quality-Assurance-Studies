...
Root --> BB["ブラックボックス技法<br/>(仕様ベース / 4.2節)"]
Root --> WB["ホワイトボックス技法<br/>(構造ベース / 4.3節)"]
Root --> EB["経験ベースの技法<br/>(4.4節)"]
BB --> B1["同値分割法 (EP)"]
BB --> B2["境界値分析 (BVA)"]
BB --> B3["デシジョンテーブルテスト"]
BB --> B4["状態遷移テスト"]
...
EB --> D3["チェックリストベースドテスト"]`} />
                    ...
                    <section id="sec-2">
                        ...
                        <h2>ブラックボックステスト技法（Black-box Test Techniques）</h2>
                        <p>
...
Locked --> [*]`} />
                    ...
                    <section id="sec-3">
                        ...
                        <h2>ホワイトボックステスト技法（White-box Test Techniques）</h2>
                        <p>
...
S3 --> Finish`} />
                    ...
                    <section id="sec-4">
                        ...
                        <h2>経験ベースのテスト技法（Experience-based Test Techniques）</h2>
                        <p>
                    ...
                    <section id="sec-4-3">
                        ...
                        <div className="diagram">
                            <Mermaid chart={`flowchart TD
A["テスターの経験・知識・直感"]
A --> B["エラー推測<br/>(欠陥を予測してピンポイントで狙う)"]
A --> C["探索的テスト<br/>(学習・設計・実行を同時に行う)"]
A --> D["チェックリストベースドテスト<br/>(網羅リストで抜け漏れを防ぐ)"]
B --> E["フォーマル技法が見逃す欠陥を検出"]
C --> E
D --> E`} />
                    ...
                    <section id="sec-5-3">
                        ...
                        <div className="diagram">
                            <Mermaid chart={`flowchart TD
A["ユーザーストーリー + 受け入れ基準"]
A --> B["受け入れテストケースの共同作成<br/>(顧客・開発者・テスター)"]
B --> C["テストの実行<br/>(実装前は当然 Fail)"]
C --> D["プロダクトコードの実装"]
D --> E["テストを再実行"]
E -->|"Fail"| D
E -->|"Pass"| F["ユーザーストーリー完了"]
F -.->|"回帰テストに追加"| G["リグレッションテストスイート"]`} />
...
Start(["テスト設計を始める"])
Start --> Q1{"仕様書・要件が明確?"}
Q1 -->|"No"| Exp["経験ベース技法<br/>(探索的テスト・エラー推測)"]
Q1 -->|"Yes"| Q2{"入力に範囲・分類がある?"}
Q2 -->|"No"| DT["デシジョンテーブルテスト<br/>(複数条件の組合せ)"]
Q2 -->|"Yes"| Q3{"境界・順序性がある?"}
Q3 -->|"Yes"| BVA["境界値分析 (BVA)<br/>+ 同値分割法 (EP)"]
Q3 -->|"No"| EP["同値分割法 (EP)"]`} />
                            <div className="diagram-caption">図6: テスト技法の選択フローチャート（仕様ベース技法）</div>
                        </div>
                    ...
                    <section id="sec-7">
                        ...
                        <div className="diagram">
                            <Mermaid chart={`flowchart TD
A["1. 用語の暗記<br/>(K1 キーワード)"]
A --> B["2. 各技法の定義理解<br/>(K2 学習目標)"]
B --> C["3. 手計算でのテストケース導出練習<br/>(K3 学習目標)"]
C --> D["4. 公式サンプル問題を解く"]
