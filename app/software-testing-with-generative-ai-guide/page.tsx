...
上記の例に倣ってGherkin形式のシナリオを3つ以上作成してください。`}</pre>
          ...
          {/* 第9章 生成AI活用のリスク管理 */}
          <section>
            <h2 id="第9章-生成ai活用のリスク管理">第9章 生成AI活用のリスク管理</h2>
            <p>
              ここまでの活用例だけを見ると生成AIは万能に思えるかもしれませんが、テスターとして最も重要なのはリスクを正しく理解し、適切な緩和策を講じることです。ISTQBのCT-GenAIシラバスも、リスク管理の章を専門の1章として独立させています[2]。
            </p>

            <h3 id="91-ハルシネーション推論エラーバイアス">
              9.1 ハルシネーション・推論エラー・バイアス
            </h3>
            <p>生成AI特有の欠陥として、次の3つを区別して理解しておく必要があります[2]。</p>
            <ul>
              <li>
                <strong>ハルシネーション</strong>：事実に反する、あるいは文脈上無関係な内容をAIが生成してしまう現象。存在しない受け入れ基準を検証するテストケースを作ってしまう、といった形で現れます。
              </li>
              <li>
                <strong>推論エラー</strong>：因果関係や条件分岐、段階的な問題解決といった論理構造をLLMが誤って解釈すること。LLMは真の論理的推論を行っているわけではなくパターンマッチングに依存しているため、テスト工数の見積もりやテストケースの優先順位付けのような論理的思考を要するタスクで誤りが生じやすくなります。
              </li>
              <li>
                <strong>バイアス</strong>：学習データに由来する偏り。特定の言語や特定の観点に偏った合成テストデータが生成されるといった形で現れます。
              </li>
            </ul>
            <p>
              これらは同じLLMであっても会話ごとに現れたり消えたりする、非決定的な性質を持つため、「一度直った」で終わらせず継続的な検証が必要です[2]。
            </p>
            <p>生成物の検証には、次のようなフローを組み込むと効果的です。</p>
            ...
            <div className="table-scroll">
              <table aria-labelledby="92-データプライバシーとセキュリティのリスク">
                ...
                <tbody>
                  <tr className="odd">
                    <td><strong>コンテキスト操作</strong></td>
                    <td>機密性のある学習データを引き出すことを意図したリクエストを送信する</td>
                    <td>長大なプロンプトでLLMのコンテキストウィンドウを圧迫しAIのメモリに負荷をかけることで、学習データの断片をランダムに出力させ、機密情報を露呈させる</td>
                  </tr>
                  <tr className="even">
                    <td><strong>入力の不正操作</strong></td>
                    <td>AIの出力を乱すようなデータを混入させる</td>
                    <td>画像などで文脈をすり替え、受け入れ基準に関するハルシネーションを誘発する</td>
                  </tr>
                  <tr className="odd">
                    <td><strong>データ汚染</strong></td>
                    <td>学習や評価に使うデータそのものを操作する</td>
                    <td>AIが生成したテストレポートの評価に虚偽の評価を混入させる</td>
                  </tr>
                  <tr className="even">
                    <td><strong>悪意あるコード生成</strong></td>
                    <td>LLMを操って裏口となるコードを生成させる</td>
                    <td>特定の不正な宛先へ通信するコードをテストコードに紛れ込ませる</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <p>データ保護の観点では、次のような基本的な緩和策が有効です[2]。</p>
            <ul>
              <li>データの最小化：必要のない機微情報は処理させない</li>
              <li>匿名化・仮名化：識別情報をマスキングまたは置換する</li>
              <li>安全な保存と通信：強固な暗号化とアクセス制御を実施する</li>
              <li>教育：組織として生成AIの責任ある利用に関する方針とトレーニングを整備する</li>
            </ul>

            <h3 id="93-環境負荷という見落とされがちな視点">
            ...
            <h3 id="94-規制標準の全体像">9.4 規制・標準の全体像</h3>
            <p>
              ...
              <table aria-labelledby="94-規制標準の全体像">
                ...
                <tbody>
                  <tr className="odd">
                    ...
                    <td>標準</td>
                    <td>組織内でAIシステムを管理するための要件を定め、生成AIを使ったテストの一貫性と信頼性を支える</td>
                  </tr>
                  <tr className="even">
                    <td><strong>ISO/IEC 23053:2022</strong></td>
                    <td>標準</td>
                    <td>機械学習を用いたAIシステムのライフサイクルの枠組みを提供し、データ品質や透明性の指針となる</td>
                  </tr>
                  <tr className="odd">
                  ...
                  <tr className="even">
                    ...
                    <td>フレームワーク</td>
                    <td>公平性・透明性・セキュリティの観点からAIのリスク管理指針を提供する</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section>
            <h2 id="第10章-コンテキストの拡張--ragとファインチューニング">
              第10章 コンテキストの拡張 ― RAGとファインチューニング
            </h2>
            <p>
            ...
            <h3 id="101-rag検索拡張生成の仕組み">10.1 RAG（検索拡張生成）の仕組み</h3>
            <p>
            ...
            <h3 id="102-ファインチューニングとの比較">10.2 ファインチューニングとの比較</h3>
            <p>
            ...
            <h3 id="103-llmopsという運用の視点">10.3 LLMOpsという運用の視点</h3>
            <p>
          ...
          {/* 第11章 組織導入とチーム変革 */}
          <section>
            <h2 id="第11章-組織導入とチーム変革">第11章 組織導入とチーム変革</h2>
            <p>
              個人としての活用スキルが身についたら、次はチームや組織としてどう広げていくかという段階に進みます。
            </p>

            <h3 id="111-shadow-aiのリスク">11.1 Shadow AIのリスク</h3>
            <p>
            ...
            <h3 id="112-段階的な導入ロードマップ">11.2 段階的な導入ロードマップ</h3>
            <p>
            ...
            <h3 id="113-dora-2025レポートが示す教訓">11.3 DORA 2025レポートが示す教訓</h3>
            <p>
