...
> 前提知識: ISTQB® Foundation Level (CTFL) 認定取得者向け(バージョンは問わず、CTFL v4.0が推奨・旧バージョンも有効)。ソフトウェア開発/テストの実務経験(目安6か月)があることが強く推奨される
...
> ✅ **ベストプラクティス:** テスト実行前には、必要に応じてスモークテストを設計・実行しテスト環境の健全性を確認する。規制業界(例:RTCA DO-178C)ではテストウェア自体が標準準拠のエビデンスとなるため、証跡管理も忘れずに行う。
...
> ✅ **ベストプラクティス:** 自動テストが失敗した際は、まず失敗の原因を分析し、必要に応じて手動または別経路での再現確認を行ったうえで欠陥として報告する。これにより自動化基盤自体の不具合(偽陽性)を欠陥として誤登録するリスクを防げる。
...
| **④ メタモルフィックテスト** | 入力の変化が結果にどう反映されるべきかという関係(MR)を使う(→3.3.2) | AIベースシステム等、データの複雑性や非決定性により費用対効果の高いテストオラクルを得にくい場合に有効 |
...
| 2 | **機密性** | 個人情報等の機密データは保護が必要。**仮名化(pseudonymization)**は識別子を人工的なものに置換する処理で、元データとの再照合可能性が残る。**匿名化(anonymization)**は識別情報の除去だけでは不十分で、残存する属性の組み合わせによる再識別リスクを評価し、個人と結び付けられない状態にする措置を指す。本番データをテスト環境へ複製する場合は、この再識別リスク評価を事前に行う必要がある。GDPR(EU)、HIPAA(米国)等の規制順守が必要な場合がある |
...
1. キーワードとそのパラメータの仕様化
2. キーワードテストケース(キーワードを使ったテストスクリプト)の仕様化
3. 前提条件・検証アクション・環境クリーンアップ等の追加ステップの仕様化
