...
│   ┌─────────────┬──────────────┬────────────────────────────┐   │
│   │  CTAL-TM    │  CTAL-TA     │  CTAL-TAE / CTAL-ATT       │   │
│   └─────────────┴──────────────┴────────────────────────────┘   │
│                         ↑                                        │
│  [Specialist Stream]                                             │
│   ┌──────────────────────────────────────────────────────────┐  │
│   │  CT-AcT（Acceptance Testing）← 本資格！                   │  │
│   │  ビジネスアナリスト・PO・テスターのための                     │  │
│   │  受け入れテスト専門資格                                       │  │
│   └──────────────────────────────────────────────────────────┘  │
│   ┌──────────┬──────────────┬────────────────┬──────────────┐  │
│   │  CT-AI   │  CT-PT       │  CT-MBT        │  CT-AuT      │  │
│   └──────────┴──────────────┴────────────────┴──────────────┘  │
│                         ↑                                        │
│  [Foundation Level]                                              │
│   ┌──────────────────────────────────────────────────────────┐  │
...
ビジネスゴールからテストへの変換フロー：
  ...
  └── 例：「ECサイトの売上を30%向上させたい」
       ↓ 分解
  ビジネスニーズ（Business Needs）
  └── 例：「チェックアウト完了率を85%以上にしたい」
       ↓ 定義
  要件 / ユーザーストーリー（Requirements / User Stories）
  └── 例：「ユーザーとして、3ステップ以内でチェックアウトできる」
       ↓ 具体化
  受け入れ基準（Acceptance Criteria）
  └── 例：「支払いページがカートページから2クリック以内でアクセスできること」
       ↓ 設計
  受け入れテスト（Acceptance Tests）
  └── 例：「カートに商品1点追加→住所入力→支払い→注文完了のフロー確認」
```
...
受け入れテストの主要ステークホルダー：
  ...
  ├─────────────────────────┼───────────────────────────────────┤
  │ プロダクトオーナー（PO）   │ 受け入れ基準の最終決定・承認        │
  │                          │ ビジネス要件の代表者                │
  ├─────────────────────────┼───────────────────────────────────┤
  │ ビジネスアナリスト（BA）   │ 要件の詳細化・受け入れ基準の作成    │
  │                          │ ステークホルダー間の橋渡し          │
  ├─────────────────────────┼───────────────────────────────────┤
  │ テスター / QA             │ テスト設計・実行・結果報告          │
  │                          │ テスト可能性の確認・品質の守護       │
  ├─────────────────────────┼───────────────────────────────────┤
  │ 開発者                   │ 技術的な実現可能性の確認            │
  │                          │ ATDD の実装                       │
  ├─────────────────────────┼───────────────────────────────────┤
  │ エンドユーザー / UAT担当   │ 実際の使い方でのテスト実行          │
  │                          │ ユーザビリティ・実務適合性の評価     │
  ├─────────────────────────┼───────────────────────────────────┤
  │ プロジェクトマネージャー   │ スケジュール・リソース管理          │
  │                          │ リリース判断の調整                  │
  └─────────────────────────┴───────────────────────────────────┘
```
...
受け入れテストで活用できる主要ツール：
  ...
  │  ツール        │ 言語          │ 特徴                     │
  ├──────────────┼─────────────┼──────────────────────────┤
  │  Cucumber    │ Ruby/Java/  │ Gherkin標準対応・広く普及   │
  │              │ JS/Python   │                           │
  ├──────────────┼─────────────┼──────────────────────────┤
