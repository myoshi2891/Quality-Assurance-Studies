...
> │  ・機能が仕様通り │  ・パフォーマンス │   ・UI/UX の一貫性     │
> │    に動作するか   │  ・セキュリティ   │   ・アクセシビリティ    │
> │  ・エラー処理     │  ・安定性         │   ・使いやすさ          │
> └──────────────────┴──────────────────┴───────────────────────┘
...
> │   └── ユースケース
> └── データアクセス層
> ├── ローカルDB（SQLite / Room / Core Data）
> └── ネットワーク通信（REST API / GraphQL）
> ↕ HTTPS/TLS
> サーバーサイド
> ├── APIゲートウェイ
> ├── バックエンドサービス
...
### 5.5 回帰テスト（Regression Testing）

コード変更後に既存機能が壊れていないことを確認します。

> 回帰テストの戦略
> ├── 完全回帰（Full Regression）: メジャーリリース時
> ├── 部分回帰（Partial Regression）: マイナー更新時
> │   └── 変更箇所に関連する機能のみテスト
> └── スモークテスト（Smoke Test）: ビルド検証
> └── 最重要機能のみを素早く確認

### 5.6 モバイル特有のテスト種類

> モバイル特有テスト
> ├── 割り込みテスト（Interruption Testing）
> │   ├── 電話着信中の動作
> │   ├── SMS受信中の動作
> │   ├── 通知バー操作中の動作
> │   └── バッテリー低下警告中の動作
...
> │ M2     │ Inadequate Supply Chain        │ サプライチェーンの不備 │
> │        │ Security                       │                      │
> │ M3     │ Insecure Authentication/       │ 不安全な認証・認可    │
> │        │ Authorization                  │                      │
> │ M4     │ Insufficient Input/Output      │ I/O検証の不足        │
> │        │ Validation                     │                      │
...
> Step 1: 対象デバイスのリストアップ
> └─ ユーザーデバイス統計・市場シェアを参考に選定

> Step 2: テストデバイスの調達
> └─ 購入 or クラウドサービス契約

> Step 3: デバイス管理システムの導入
> └─ MDM（Mobile Device Management）ツールの設定

> Step 4: テスト用データ・アカウントの準備
> └─ テスト専用の認証情報・データセット作成
...
> ┌──────────────┬────────────────┬────────────┬────────────┬────────────┐
> │  フレームワーク │  対応プラット   │  使用言語   │  速度      │  難易度    │
> │              │  フォーム       │            │            │            │
> ├──────────────┼────────────────┼────────────┼────────────┼────────────┤
> │ Appium       │ Android + iOS  │ Java/Python │ ⭐⭐⭐      │ 中〜高     │
> │              │ + Web          │ /JS/Ruby等  │            │            │
> ├──────────────┼────────────────┼────────────┼────────────┼────────────┤
...
> ├──────────────┼────────────────┼────────────┼────────────┼────────────┤
> │ Detox        │ Android + iOS  │ JavaScript  │ ⭐⭐⭐⭐     │ 中         │
> │              │ (React Native) │            │            │            │
> ├──────────────┼────────────────┼────────────┼────────────┼────────────┤
