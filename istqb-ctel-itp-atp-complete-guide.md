...
```
╔══════════════════════════════════════════════════════════════════════╗
║           ISTQB® 認定資格ロードマップ（Expert Level）                  ║
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  [Expert Level – ITP（Improving the Test Process）]                  ║
║   ┌─────────────────────────────────────────────────┐               ║
║   │ Part 1: CTEL-ITP-ATP                             │ ← 本ガイドの対象║
║   │ Assessing Test Processes（テストプロセスの評価）   │               ║
║   ├─────────────────────────────────────────────────┤               ║
║   │ Part 2: CTEL-ITP-ITPI                            │               ║
║   │ Implementing Test Process Improvement            │               ║
║   │ （テストプロセス改善の実装）                        │               ║
║   └─────────────────────────────────────────────────┘               ║
║   ★ 両方のパートに合格 → CTEL-ITP 正式認定（有効期限 7年）            ║
║                                                                      ║
║  [Advanced Level] ← 前提条件                                         ║
║   ┌──────────────────────────────────────────────────┐              ║
║   │  CTAL-TM（Advanced Level Test Manager）必須       │              ║
║   └──────────────────────────────────────────────────┘              ║
║                                                                      ║
║  [Foundation Level] ← 必須前提                                       ║
║   ┌──────────────────────────────────────────────────┐              ║
║   │  CTFL v4.0（Foundation Level）必須                │              ║
║   └──────────────────────────────────────────────────┘              ║
╚══════════════════════════════════════════════════════════════════════╝
```
...
改善の文脈（Context）：
  ...
  ┌─────────────────────────────────────────────┐
  │  TQM / ISO 9000 / EFQM / Six Sigma          │
  │  （ビジネス・組織改善の文脈）                 │
  │     ↕ 相互影響                              │
  │  CMMI / ISO 15504 / ITIL / TSP / PSP        │
  │  （IT・ソフトウェアプロセス改善の文脈）        │
  │     ↕ 相互影響                              │
...
デミングサイクル（PDCA）：
  ...
  Plan: 品質特性・コスト・サービスレベルの目標設定
        現状分析 → 改善計画の立案
  
  Do: 計画に基づく活動の実行
      人材への投資（トレーニング・コーチング）
  
  Check: 特定メトリクスでのコントロールポイント追跡
         実績 vs 予測の比較・偏差の観察
  
  Act: パフォーマンス向上の機会特定・優先付け
       統計的手法・システム分析の活用
```
...
│ アプローチ       │ 内容・特徴                                         │
├────────────────┼──────────────────────────────────────────────────┤
│ モデルベース    │ TMMi・TPI Nextなど定義されたモデルに沿って改善       │
│ (Model-based)  │ ベストプラクティスを段階的に適用                     │
│                │ 例：TMMi Level 3 達成を目標とする                   │
├────────────────┼──────────────────────────────────────────────────┤
│ 分析ベース     │ 実際の問題・データ分析から改善点を導き出す            │
│ (Analytical)   │ GQM・根本原因分析などを活用                         │
│                │ 例：欠陥原因分析でテスト見直しを実施                 │
├────────────────┼──────────────────────────────────────────────────┤
│ ハイブリッド   │ 上記2つを組み合わせたアプローチ                      │
│ (Hybrid)       │ 成熟度の高いプロジェクトの実践を他に展開             │
│                │ 例：TMMi内で根本原因分析を活用                       │
└────────────────┴──────────────────────────────────────────────────┘
```
...
├─────────────────────────┼───────────────────────────────────────────┤
│ 定義された成熟度レベル     │ 処方された成熟度レベルなし                  │
│ (TMMi: L1〜L5)          │                                          │
│ 段階的に上位へ進む        │ 任意の領域から改善を開始できる              │
│                          │ 異なる速度で複数領域を改善できる            │
├─────────────────────────┼───────────────────────────────────────────┤
│ ✓ 概念がシンプル          │ ✓ 柔軟性が高い                           │
│ ✓ 成熟度レベルが          │ ✓ 特定の問題領域に集中できる              │
│   外部コミュニケーション   │ ✓「全か無か」問題を回避                   │
│   に使いやすい            │                                          │
├─────────────────────────┼───────────────────────────────────────────┤
│ ✗ 柔軟性が限られる        │ ✗ 管理上の複雑さが増す                   │
│ ✗「全か無か」になりがち   │                                          │
...
TPI Next の基本構造：
  ...
  → 「偏った改善」を防ぐ（例：メトリクスを高めるには
    欠陥管理・レポーティングも一定レベル必要）
```
...
TMMi の基本構造（段階的表現）：
  ...
  │              TMMi の5段階成熟度レベル                            │
  ├────────────────────┬───────────────────────────────────────────┤
  │ Level 1: Initial   │ テスト=デバッグ。アドホックで非構造化          │
  │（初期）             │ プロセスなし・ドキュメントなし                │
  ├────────────────────┼───────────────────────────────────────────┤
  │ Level 2: Managed   │ テストがデバッグから分離される                │
  │（管理）             │ プロセスエリア：                            │
  ...
  │                    │ ・Test Environment                         │
  ├────────────────────┼───────────────────────────────────────────┤
  │ Level 3: Defined   │ テストプロセスが組織全体で標準化              │
  │（定義）             │ プロセスエリア：                            │
  │                    │ ・Test Organization                        │
  ...
  │                    │ ・Peer Reviews                             │
  ├────────────────────┼───────────────────────────────────────────┤
  │ Level 4:           │ 定量的管理・測定プログラム                   │
  │ Management and     │ プロセスエリア：                            │
  │ Measurement        │ ・Test Measurement                         │
  │（管理・測定）        │ ・Software Quality Evaluation             │
  │                    │ ・Advanced Peer Reviews                    │
  ├────────────────────┼───────────────────────────────────────────┤
  │ Level 5:           │ 継続的最適化・欠陥防止                      │
  │ Optimization       │ プロセスエリア：                            │
  │（最適化）           │ ・Defect Prevention                        │
  │                    │ ・Quality Control                          │
...
├──────────────────┼───────────────────────┼──────────────────────── ┤
│ SPI との関係      │ 正式なSPIモデルとの     │ CMMIと高度に相関        │
│                  │ 関係なし（マッピング可）│                        │
├──────────────────┼───────────────────────┼──────────────────────── ┤
│ フォーカス        │ 16のキーエリア          │ 成熟度レベルごとの      │
│                  │ テスト固有の詳細視点    │ プロセスエリア          │
├──────────────────┼───────────────────────┼──────────────────────── ┤
│ 強み              │ ビジネス主導・          │ 管理コミットメントへの  │
│                  │ テストエンジニアリング  │ 強いフォーカス          │
│                  │ の両方をカバー          │ CMMIとの相関が高い      │
└──────────────────┴───────────────────────┴──────────────────────── ┘
```
...
分析ベースアプローチの特徴：
  ...
  ✓ 問題ベース（Problem-based）：
    → 「ベストプラクティス」の汎用モデルではなく
      実際の問題・目標に基づいて改善を決定する
  ...
  │ モデルベース       │ 分析ベース                        │
  ├───────────────────┼─────────────────────────────────┤
  │ 定義された「ベスト │ 実際の問題から改善点を特定        │
  │ プラクティス」を   │                                 │
  │ 参照する           │                                 │
  │                   │                                 │
  │ 組織の現状を       │ プロジェクト固有のゴールを        │
  │ モデルと比較する   │ 定義・測定する                   │
  └───────────────────┴─────────────────────────────────┘
```
...
フィッシュボーン図の適用手順（Robson 1995 より）：
  ...
  STEP 6: 原因・症状のクラスターを分析
          パレート分析（20%の努力で80%の改善）で
          優先すべきクラスターを特定

具体例（テストの問題への適用）：

  問題（Effect）：「本番環境での欠陥率が高い」
                              ↑
  ┌──────────────────────────┐│
  │ People    ：スキル不足     ││
  │ Process   ：テスト設計不足 ││
...
GQM の3レベル（Basili の GQM アプローチ）：
  ...
  ├─────────────────┼───────────────────────────────┤
  │ Level 2:        │ 運用レベル                      │
  │ Question        │ 品質に関してオブジェクトを特性  │
  │ （質問）         │ 付けする質問                   │
  ├─────────────────┼───────────────────────────────┤
  │ Level 3: Metric │ 定量的レベル                    │
  │ （メトリクス）   │ 客観的（定量・事実）または      │
  │                 │ 主観的（定性・視点）            │
  └─────────────────┴───────────────────────────────┘
...
│  ✓ テストプロセスがすでに存在する場合（設立にも使える）               │
│  ✓ 類似プロジェクト間での比較・ベンチマークが必要                    │
│  ✓ SPIモデルとの互換性が必要                                        │
│  ✓ 会社ポリシーとして特定成熟度レベル達成が目標                      │
│  ✓ 明確な出発点と規定された改善パスが望ましい                        │
│  ✓ テスト成熟度の測定がマーケティング目的で必要                      │
│                                                                     │
│  コンテンツモデル（CTP / STEP）を使う場面：                            │
│  ✓ テストプロセスを新規構築する必要がある                            │
│  ✓ 現行テストプロセスのコスト・リスク評価が必要                      │
│  ✓ TMMi / TPI Next の順序ではなくビジネスニーズ順で改善したい        │
│  ✓ 会社固有のコンテキストへのテーラリングが必要                      │
│  ✓ 非連続的・急速な改善が必要                                        │
│                                                                     │
│  分析的アプローチを使う場面：                                          │
│  ✓ 特定の問題を対象にする必要がある                                  │
│  ✓ 測定値・メトリクスが利用可能（または確立できる）                   │
│  ✓ テストプロセスの必要性の証拠が必要                                │
│  ✓ 変更の理由について合意が必要                                      │
│  ✓ 問題の根本原因がテストプロセスオーナーの管理外の場合              │
│  ✓ 小規模の評価・改善パイロットが予算の範囲内の場合                  │
│  ✓ 組織文化が内部分析による証拠を外部モデルより信頼する場合          │
└─────────────────────────────────────────────────────────────────────┘
```
...
変更への人間の反応（典型的なパターン）：
  ...
  ┌─────────────────────────────────────────────────────────────┐
  │  衝撃・否定 → 抵抗 → 探索 → コミットメント                  │
  │  (Shock/   (Resist)(Explore)(Commitment)                   │
  │  Denial)                                                    │
  └─────────────────────────────────────────────────────────────┘
...
✅ TMMi の5レベル：
   ...
   Management & Measurement(4) → Optimization(5)

✅ TMMi Level 2 の5つのプロセスエリア：
   Test Policy and Strategy / Test Planning / Test Monitoring and Control /
   Test Design and Execution / Test Environment

✅ TPI Next の16キーエリア（全名称）

✅ TMMi（Staged）vs TPI Next（Continuous）の違い

✅ 品質の5つのビュー：
   製品ベース / 製造ベース / ユーザーベース / 価値ベース / 超越的

✅ GQM の3レベル：
   Goal（ゴール）→ Question（質問）→ Metric（メトリクス）

✅ DDP の計算式：
   DDP = テスト検出欠陥数 / 全既知欠陥数 × 100%

✅ 因果分析（フィッシュボーン）の6ステップ

✅ アプローチ選択基準（モデルベース vs コンテンツ vs 分析）

✅ 段階的 vs 継続的表現の比較（メリット・デメリット）

✅ EFQM の8つの基本概念

✅ テストプロセス改善の範囲（プロセス本体・インフラ・組織）
...
```
✅ Chapter 2（コンテキスト）:
□ テスト改善の典型的なビジネス理由を5つ以上言える
□ 品質の5ビューを全て説明し、テストへの適用を説明できる
□ デミングサイクルの4ステップを説明できる
□ IDEAL フレームワークの5フェーズを順に説明できる
□ EFQM の8基本概念を説明できる
□ モデルベース / 分析ベース / ハイブリッドの違いを説明できる

✅ Chapter 3（モデルベース改善）:
□ 段階的表現 vs 継続的表現のメリット・デメリットを比較できる
□ TMMi の5レベルを全て説明できる
□ TMMi Level 2 の5つのプロセスエリアを全て挙げられる
□ TMMi Level 3 の5つのプロセスエリアを全て挙げられる
□ TPI Next の16キーエリアを全て挙げられる
□ TPI Next vs TMMi の6つの観点で比較できる
□ STEP と CTP の違いを説明できる
□ CMMI の段階的・継続的表現の違いを説明できる
□ ISO/IEC 15504 の概要を説明できる

✅ Chapter 4（分析ベース改善）:
□ フィッシュボーン図の適用手順6ステップを説明できる
□ インスペクション因果分析（Part 1: 90分 / Part 2: 30分）を説明できる
□ GQM の3レベル（Goal / Question / Metric）を説明できる
□ DDP を計算できる（DDP = テスト検出 / 全既知欠陥 × 100%）
□ テスト効率・コストメトリクスを3つ以上説明できる
□ リードタイム・予測可能性メトリクスを説明できる

✅ Chapter 5（アプローチ選択）:
□ プロセスモデルを使う7つの条件を説明できる
□ コンテンツモデルを使う5つの条件を説明できる
□ 分析的アプローチを使う8つの条件を説明できる

✅ Chapter 6（改善プロセス）:
□ テストポリシーの主要要素を説明できる
□ IDEAL「Initiating」の活動を説明できる
□ 改善の必要性を示す主要ステークホルダー5タイプを挙げられる
□ 改善目標設定の3ステップを説明できる
□ バランスドスコアカードとの整合の例を説明できる
□ プロジェクト中心 vs 組織中心の違いを説明できる
□ 評価計画の内容（インタビュー対象者7役割含む）を説明できる
□ 評価レポートの最低限の内容を説明できる
□ ソリューション分析の5アプローチを説明できる
□ トップダウン vs ボトムアップの違いを説明できる
□ テスト改善計画の典型的な内容を説明できる

✅ Chapter 7（組織・役割・スキル）:
□ TPG の主な活動を説明できる
□ インタビュー質問の4タイプ（開放型・閉鎖型・深掘り・仮定型）を説明できる
□ テストプロセス改善者の必須スキル7種類を説明できる

✅ Chapter 8（変更管理）:
□ 変更への人間の典型的な反応段階を説明できる
□ 抵抗の一般的な原因と対処法を説明できる

✅ Chapter 9（重要成功要因）:
□ テストプロセス改善の主要成功要因7つを説明できる
□ 改善文化を形成するための要素を説明できる
```
