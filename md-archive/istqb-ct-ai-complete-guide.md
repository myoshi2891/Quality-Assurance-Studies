...
```mermaid
flowchart TD
    subgraph Specialist[スペシャリスト資格]
        AI["CT-AI (Certified Tester AI Testing)
        ★本資格！
...
| 観点 | CT-AI（2021） | CT-GenAI（2025） |
|------|--------------|----------------|
| 主目的 | AIシステムをテストする | 生成AIを使ってテストする |
| 技術領域 | ML・ニューラルネット | LLM・プロンプトエンジニアリング |
| 品質観点 | バイアス・倫理・XAI | RAG・エージェント・LLMOps |
| メトリクス | 混同行列・精度・再現率 | ハルシネーション・プライバシー |
...
- 例：Deep Blue がチェスで人間を倒した後、「あれはブルートフォースで本当のAIではない」と言われた
...
| 種類 | 別名 | 特徴 | 現在の状況 |
|------|------|------|-----------|
| 狭義AI（Narrow AI） | 弱いAI（Weak AI） | 特定タスクに特化、文脈が限定的 | ✅ 広く普及（検索・推薦等） |
| 汎用AI（General AI） | 強いAI（Strong AI） | 人間同様の汎用認知・理解・推論・適応 | ❌ 未実現（2021年時点） |
| 超AI（Super AI） | Super AI | 人間を超える認知・無限の処理・知識、技術的特異点を超えた | ❌ 未実現（テクノロジー特異点の先） |
...
> ※ テスター視点：これらのフレームワークを「認識」できれば十分。深い実装知識は CT-AI 試験では不要。
...
**機械学習の3つの主要な形態（ML の種類比較）**

| 観点 | 教師あり学習（Supervised） | 教師なし学習（Unsupervised） | 強化学習（Reinforcement） |
|------|--------------------------|---------------------------|------------------------|
| データ | ラベル付きデータ・正解ありで学習 | ラベルなしデータ・構造・パターンを発見 | 環境との試行錯誤・報酬を最大化 |
| タスク | 分類・回帰 | クラスタリング・次元削減 | ゲームAI・自動制御 |
| 代表例 | スパムフィルター・医療診断AI・画像分類 | 顧客セグメンテーション・異常検知・推薦システム | AlphaGo・ロボット制御・自動運転 |
...
| データセット | 用途 | 目安比率 |
|------------|------|---------|
| トレーニング（Training） | モデルの学習・パラメータ調整 | 70-80% |
| バリデーション（Validation） | ハイパーパラメータ調整・モデル選択・過学習の早期検出 | 10-15% |
| テスト（Test） | 最終性能評価（一度だけ使用）・モデルの汎化性能を評価 | 10-20% |
