...
describe('Cucumber Beginner Guide NavBar Component', () => {
  ...
  it('re-evaluates section visibility on scroll so the active link follows a ratio reversal', async () => {
    ...
    const rects: Record<string, { top: number; bottom: number }> = {
      ...
      'sec01': { top: bandTop - 100, bottom: bandTop + bandHeight * 0.75 },
      'sec02': { top: bandTop + bandHeight * 0.75, bottom: bandBottom + 400 },
    };
    ...
    try {
      ...
      await waitFor(() => {
        expect(container.querySelector('#toc a.active')?.getAttribute('href')).toBe('#sec01');
      });

      // スクロールで可視率が逆転（帯の 25% / 75%）。交差状態自体は両節とも維持される。
      rects['sec01'] = { top: bandTop - 400, bottom: bandTop + bandHeight * 0.25 };
      rects['sec02'] = { top: bandTop + bandHeight * 0.25, bottom: bandBottom + 100 };
      act(() => {
      ...
      await waitFor(() => {
        ...
        expect(active?.getAttribute('href')).toBe('#sec02');
