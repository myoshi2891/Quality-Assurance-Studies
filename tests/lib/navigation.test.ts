...
describe('NAV_ITEMS', () => {
  it('contains 78 entries (home + 9 foundation + 11 fdn-ext + 10 advanced + 14 specialist + 5 expert + 2 cicd-devops + 8 tools-frameworks + 18 books-practices)', () => {
    expect(NAV_ITEMS).toHaveLength(78);
  });
  ...
  it('classifies /the-way-of-the-web-tester-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/the-way-of-the-web-tester-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });

  it('classifies /testing-web-apis-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/testing-web-apis-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });
  ...
  it('classifies /art-of-software-testing-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/art-of-software-testing-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });

  it('classifies /test-driven-development-by-example-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/test-driven-development-by-example-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });

  it('classifies /unit-testing-principles-practices-patterns-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/unit-testing-principles-practices-patterns-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });

  it('classifies /perfect-software-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/perfect-software-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });

  it('classifies /ai-driven-software-testing-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/ai-driven-software-testing-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
  });

  it('classifies /testing-ai-confidence-engineering-guide as books-practices', () => {
    const book = NAV_ITEMS.find((item: NavItem) => item.href === '/testing-ai-confidence-engineering-guide');
    expect(book).toBeDefined();
    expect(book?.category).toBe('books-practices');
...
describe('groupByCategory', () => {
  ...
  it('classifies /software-testing-with-generative-ai-guide as books-practices', () => {
    const item = NAV_ITEMS.find((i: NavItem) => i.href === '/software-testing-with-generative-ai-guide');
    expect(item?.category).toBe('books-practices');
  });
  ...
  it('preserves the original item order within each group', () => {
    ...
    expect(advanced?.items.map((i) => i.href)).toEqual([
      ...
      '/istqb-ctal-ta-chapter3-test-analysis-and-design',
      '/istqb-ctal-ta-chapter4-quality-characteristics',
